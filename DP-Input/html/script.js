...
const OpenMenu = (data) => {
    ...
    let form = ["<form id='DP-Input-form'>", `<div class="heading">${data.header != null ? data.header : "Form Title"}</div>`];
    ...
    $("#DP-Input-form").on("change", function (event) {
    ...
    $("#DP-Input-form").on("submit", async function (event) {
        if (event != null) {
...
const renderRadioInput = (item) => {
    ...
    options.forEach((option, index) => {
        const isChecked = (item.default ? item.default == option.value : index == 0) ? "checked" : "";
        div += `
        <label class="radio-label" for="radio_${name}_${index}">
            <input type="radio" id="radio_${name}_${index}" name="${name}" value="${option.value}" ${isChecked}>
...
const renderColorInput = (item) => {
    const { text, name } = item;
    formInputs[name] = item.default ? item.default : "#0ea021;";
    const isRequired = item.isRequired == "true" || item.isRequired ? "required" : "";
    const defaultValue = item.default ? `value="${item.default}"` : "";
